...
/*EN - When "startGame.html" is initially launched, an information modal is displayed on the screen.*/

/*TR - Ekranda modal dışındaki bir yere tıklandığında bilgilendirme metnine ait modalı kapatır.*/
/*EN - Clicking anywhere outside the modal closes the modal for the information message.*/
...
/*EN - It creates an object with the value of "true" for the "accepted" key in local storage when the user clicks the button in the information message, and closes the message.*/
...
/*EN - If the value of the accepted key is not true, the informative modal will be opened again on page load and in the mouseover event of the small black square.*/
...
/*EN - If the value of the "accepted" key is "true", the main screen (start game screen) will be transitioned to in the mouseover event of the small black square. Necessary animations will be played and a few hidden divs will become visible.*/
...
/*EN - Function that returns the high score data.*/
function getHighScore() {
	return localStorage.getItem("highScore");
}

/*TR - Local storage dan çağırılan nesneleri getirir.*/
/*EN - Retrieves the objects called from local storage.*/
...
/*EN - The variables to be used at the start of the game.*/
var started = false;
var score = 0;
...
/*EN - It changes the text on the square that says "start game" to "game starting", plays a sound, and displays a progress bar animation. After 2400 milliseconds, it opens the square indicating that the game has started and disables the appearance and functions of the "settings" and "how to play?" buttons.*/
...
/*EN - When the "settings" button is clicked, it opens the settings screen. It disables the click functions of the "start game" square that prevents the game from starting.*/
...
/*EN - Saves the settings and refreshes the page after 800 milliseconds. (This is necessary for some functions to work.)*/
...
/*EN - The function called when the square saying "start game" is clicked. It writes the HTML texts belonging to the specified ids. "randomNumber1" is generated and written on the left side of the screen. "randomNumber2" is hidden with "??"" and the HTML text is written.*/
function startGame() {
	$("#result").html("");
...
/*EN - Pass the "up" parameter to the Calculate function.*/
$("#upBtn").click(function () {
	calculate("up");
});

/*TR - Calculate fonksiyonuna "down" parametresini taşır.*/
/*EN - Pass the "down" parameter to the Calculate function.*/
$("#downBtn").click(function () {
	calculate("down");
});

/*TR - Cevap verildiğinde "up" ve "down" butonlarını gizler.*/
/*EN - It hides the "up" and "down" buttons when the answer is given.*/
...
/*EN - The function generates "randomNumber2" here based on the parameters coming from the up and down buttons. It compares "randomNumber1" and "randomNumber2" and calls the function that needs to be called according to the result.*/
...
/*EN - It plays the necessary sounds and creates animations when a round is won. It increments the score by 1. If the score is greater than the high score, it performs the assignment of the new high score.*/
...
/*EN - When the game is lost, it plays the necessary sounds and creates animations. It unhides the "restart" and "exit" buttons.*/
...
/*EN - When the game is lost, it calls the "startGame" function to restart the game. Score data is cleared, up and down buttons are shown again. Exit and restart buttons are hidden.*/
...
/*EN - Enabling the player to return to the main menu by clicking on the "exit" button when the game is lost.*/
...
/*EN - In case of a tie, it writes the necessary HTML texts. Since it is a very low possibility, it counts as a win for the player and adds 10 points. Then it calls the "winProgress" function.*/
...
/*EN - The function adds the animation specified in the second parameter to the HTML element with the given id. It creates the animation visually.*/
...
/*EN - Function that plays the sound files received as a parameter.*/
...
/*EN - A function used for testing purposes that deletes the local storage objects captured with a parameter using only the console.*/
...
/*EN - Starts the progress bar animation that will appear below the "game starting" text.*/
...
/*EN - When an option in the settings tab is clicked, it is called. It adds style to the options and saves the selections to local storage.*/
...
$("#languageTr").on("click", function () {
	$(this).addClass("blueColor");
	$("#languageEn").removeClass("blueColor");
...
/*EN - Add style to the saved options of the settings when the page is loaded. (The texts become blue.)*/
$(window).on("load", function () {
	if (soundSetting === "on") {
	...
	if (languageSetting === "tr") {
		...
		$("#languageEn").toggleClass("blueColor");
	}
...
/*EN - Add style to the sound settings options when first launched. (Text appears in blue.)*/
...
/*EN - When the delete high scores option is clicked, it shows the delete and cancel options.*/
...
/*EN - When clicked on the cancel option, it closes the options.*/
...
/*EN - When the delete option is clicked, it deletes the score data from local storage.*/
